...
st.markdown("**多模型集成预测 + RAG知识库检索 + 智能工艺优化 + 可视化分析**")
...
# 必填特征（用于模型训练，缺失值会填充中位数）
REQUIRED_FEATURES = [
    'ash', 'volatile', 'carbon_temp', 'hold_time', 'heating_rate',
    'activation_temp', 'activation_time', 'activator_ratio'
]

# 可选特征（有则用，无则忽略）
OPTIONAL_FEATURES = [
    'fixed_carbon', 'carbon_content', 'hydrogen_content', 'oxygen_content',
    'vitrinite_content', 'acid_concentration', 'acid_temp', 'acid_time',
    'preoxidation_temp', 'preoxidation_time',
    'd002', 'La', 'Lc', 'id_ig', 'ssa', 'micropore_volume'
]
...
@st.cache_resource
def load_data():
    df = pd.read_csv("data.csv", encoding='utf-8-sig', on_bad_lines='skip')
...
# ========== 2. Sentence Embedding 模型（用于 RAG 语义检索） ==========
@st.cache_resource
    ...
    for val in param_range:
        ...
        for i, feat in enumerate(available_features):
            if feat == param_name:
                ...
                input_dict[feat] = fixed_values.get(feat, df[feat].median())
        input_df = pd.DataFrame([input_dict])[available_features]
        input_scaled = scaler.transform(input_df)
...
def build_input_vector():
    """根据用户输入构造特征向量（与 available_features 对齐）"""
    input_dict = {}
    for feat in available_features:
        if feat == 'ash':
            input_dict[feat] = ash
        elif feat == 'volatile':
            input_dict[feat] = volatile
        elif feat == 'fixed_carbon':
            input_dict[feat] = fixed_carbon
        elif feat == 'carbon_content':
            input_dict[feat] = carbon_content
        elif feat == 'hydrogen_content':
            input_dict[feat] = hydrogen_content
        elif feat == 'oxygen_content':
            input_dict[feat] = oxygen_content
        elif feat == 'vitrinite_content':
            input_dict[feat] = vitrinite_content
        elif feat == 'acid_concentration':
            input_dict[feat] = acid_concentration
        elif feat == 'acid_temp':
            input_dict[feat] = acid_temp
        elif feat == 'acid_time':
            input_dict[feat] = acid_time
        elif feat == 'preoxidation_temp':
            input_dict[feat] = preoxidation_temp
        elif feat == 'preoxidation_time':
            input_dict[feat] = preoxidation_time
        elif feat == 'carbon_temp':
            input_dict[feat] = carbon_temp
        elif feat == 'hold_time':
            input_dict[feat] = hold_time
        elif feat == 'heating_rate':
            input_dict[feat] = heating_rate
        elif feat == 'activation_temp':
            input_dict[feat] = activation_temp
        elif feat == 'activation_time':
            input_dict[feat] = activation_time
        elif feat == 'activator_ratio':
            input_dict[feat] = activator_ratio
        elif feat == 'd002':
            input_dict[feat] = d002 if use_structure else df[feat].median()
        elif feat == 'La':
            input_dict[feat] = La if use_structure else df[feat].median()
        elif feat == 'Lc':
            input_dict[feat] = Lc if use_structure else df[feat].median()
        elif feat == 'id_ig':
            input_dict[feat] = id_ig if use_structure else df[feat].median()
        elif feat == 'ssa':
            input_dict[feat] = ssa if use_structure else df[feat].median()
        elif feat == 'micropore_volume':
            input_dict[feat] = micropore_volume if use_structure else df[feat].median()
        else:
            input_dict[feat] = 0.0
    return [input_dict[f] for f in available_features]

# ========== 10. 主区域 Tabs ==========
tab1, tab2, tab3, tab4 = st.tabs(["📊 智能预测", "📚 RAG知识检索", "📈 可视化分析", "💡 工艺优化"])
...
# ========== 底部 ==========
st.markdown("---")
st.caption("煤基硬碳AI工艺智能体 v2.0 | 多模型集成 + RAG语义检索 + 可视化分析 | 完整参数版本 | Powered by Streamlit")
