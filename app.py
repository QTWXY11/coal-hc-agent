...
def load_data():
    ...
    # ---- 步骤1：尝试多种编码读取，并自动定位表头行 ----
    encodings = ['utf-8-sig', 'gbk', 'gb18030', 'utf-8', 'latin1']
    raw_df = None
    used_encoding = None
    for enc in encodings:
        try:
            raw_df = pd.read_csv("data.csv", encoding=enc, on_bad_lines='skip', header=None, dtype=str)
            used_encoding = enc
            break
        except Exception:
            continue
    ...
    if raw_df is None:
        ...
        try:
            ...
            st.error(f"❌ 无法读取数据文件。请确保 data.csv 存在。\n错误：{e}")
            st.stop()
    else:
        # 找到包含 "sample_id" 的行作为表头
        header_row = 0
    ...
    if X.shape[1] == 0:
        st.error("❌ 没有有效特征可用于训练。")
        st.stop()

    y = df['capacity'].fillna(df['capacity'].median())
    if y.isnull().all():
        st.error("❌ 'capacity' 列全为空，无法训练。")
        st.stop()
    ...
    atmosphere_options = sorted([str(x) for x in df['atmosphere'].dropna().unique().tolist() if str(x) != '无'])
...
def call_llm(prompt):
    ...
    try:
        ...
        elif resp.status_code == 403 and "AllocationQuota.FreeTierOnly" in resp.text:
            return ("⚠️ **阿里云百炼免费额度已用完！**\n\n"
                    "请前往 [阿里云百炼控制台](https://bailian.console.aliyun.com/) 充值，"
                    "并关闭「仅使用免费额度」模式；或更换为其他 API（如 DeepSeek）。")
        else:
